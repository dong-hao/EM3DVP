function select_site(hObject,eventdata,handles,opt,button)
...
if strcmp(opt,'user')  % i.e. user-invoked.  
    ...
    if ~isempty(b)&&strcmp(opt,'off')  % function being de-invoked
        ...
        if ~isempty(get(h,'zdata'))
            error('select_site only works for 2-D graphs')
        end
        ...
        set(h,'buttondownfcn',{@select_site,handles,'self',1},'userdata',k,'tag','select_site');
    ...
    if strcmp(get(gcf,'selectiontype'),'open')
        ...
        case 1 % line's buttondownfcn invoked
            ...
            set(h,'userdata',k);
            set(gca,'userdata',h);
            set(gcf,'windowbuttonupfcn',{@select_site,handles,opt,2});
        case 2 % button up - we're done
            ...
            if get(handles.Zbox(1),'value')==1
                plot_sounding(hObject,eventdata,handles,'xxyy');
                set(handles.Zbox,'value',0);
                set(handles.Zbox(1),'value',1);
            elseif get(handles.Zbox(2),'value')==1
                plot_sounding(hObject,eventdata,handles,'xyyx');
                set(handles.Zbox,'value',0);
                set(handles.Zbox(2),'value',1);
            else
                plot_sounding(hObject,eventdata,handles,'txty');
                set(handles.Zbox,'value',0);
...
%==========create RMS viewer gui===========%
%
%         view RMS variant of iterations
%
%==========================================%
