function set_rho_area(hObject,eventdata,h)
...
if initA>finalA-1||initB>finalB-1
    ...
    switch view
        case 1
            BlockPara=blockdlg(length(model.y)-1,custom.currentY);
            model.rho(initA:finalA-1,BlockPara(1):BlockPara(2),initB:finalB-1)=BlockPara(3); 
        case 2
            BlockPara=blockdlg(length(model.x)-1,custom.currentX);
            model.rho(BlockPara(1):BlockPara(2),initA:finalA-1,initB:finalB-1)=BlockPara(3);
        case 3
            BlockPara=blockdlg(size(model.z,1)-1,custom.currentZ);
            model.rho(initB:finalB-1,initA:finalA-1,BlockPara(1):BlockPara(2))=BlockPara(3);
    end
